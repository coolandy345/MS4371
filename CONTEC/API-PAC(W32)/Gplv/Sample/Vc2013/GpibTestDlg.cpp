...
BOOL CGpibTestDlg::OnInitDialog()
{
	...
	m_Address.SetWindowText(_T("1"));
	m_Master.SetWindowText(_T("*IDN?"));
	m_Slave.SetWindowText(_T(""));
	m_SPoll.SetWindowText(_T("0"));
...
void CGpibTestDlg::OnInit() 
{
	...
	if ((ibsta & ERR) != 0) {
         MessageBox(_T("Unable to open device"));
    }
}
...
void CGpibTestDlg::OnMaster() 
{
	...
	m_Master.GetWindowText(tmp,sizeof(tmp));
	sprintf_s(wrtbuf, "%S", tmp);

	nCnt = lstrlen(tmp);
    ibwrt (Dev, wrtbuf, nCnt);

	if ((ibsta & ERR) != 0) {
        MessageBox(_T("Unable to write to device"));
    }
}
...
void CGpibTestDlg::OnSlave() 
{
	...
	ibrd (Dev, tmp, sizeof(tmp));

	wsprintf(rdbuf, _T("%S"), tmp);

	if ((ibsta & ERR) != 0) {
        MessageBox(_T("Unable to read from device"));
    }
...
void CGpibTestDlg::OnSPoll() 
{
	...
	if ((ibsta & ERR) != 0) {
        MessageBox(_T("Unable to serial poll"));
    }

	wsprintf(szBuff, _T("%x H"), (int)spr);
